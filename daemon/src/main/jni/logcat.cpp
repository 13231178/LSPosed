...
void Logcat::ProcessBuffer(struct log_msg *buf) {
    ...
    bool shortcut = false;
    if (tag == "LPP_Bridge"sv || tag == "XSharedPreferences"sv || tag == "LPPContext")
        [[unlikely]] {
    ...
    if (verbose_ && (shortcut || buf->id() == log_id::LOG_ID_CRASH || entry.pid == my_pid_ ||
                     ...
                     tag.starts_with("LPP"sv) || tag == "Magisk"sv || tag == "SELinux"sv ||
                     tag == "TEESimulator"sv || tag.starts_with("zygisk"sv))) [[unlikely]] {
        verbose_print_count_ += PrintLogLine(entry, verbose_file_.get());
    }
    if (entry.pid == my_pid_ && tag == "LPPLogcat"sv) [[unlikely]] {
